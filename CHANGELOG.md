...
- Closed six detection coverage gaps found in a round-4 audit (whole new threat categories, not bypasses of existing rules): reverse shells (`/dev/tcp` redirects, `nc -e`), macOS Keychain extraction (`security dump-keychain`/`find-*-password`), broader credential files (`.kube/config`, `.docker/config.json`, `.npmrc`, `.git-credentials`, `.pypirc`, `~/.config/gh`, and `.env` / `.env.local` while letting `.env.example` templates through), setuid/setgid escalation (`chmod u+s`, `chmod 4755`), persistence (`launchctl load`, `crontab`, writes to `Library/Launch{Agents,Daemons}`), and tampering with Sentry's own config (`> ~/.agentsentry/safety-rules.json`). Also tightened the recursive-`chmod`/`chown` rule to only flag absolute system paths, so `chmod -R u+rw build` on a project dir no longer false-positives. `tests/test-bypass.sh` grows to 97 cases.
