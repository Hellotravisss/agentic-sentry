...
if ! sentry_check_command "$CMD" "$CWD"; then
    # Safe command (or engine unavailable): stay invisible — except for the
    # retry-loop signal (T9), which logs once per window and never blocks.
    if [[ -f "$PROJECT_DIR/sentry-rate.sh" ]]; then
        ...
        if command -v sentry_rate_check >/dev/null 2>&1 && sentry_rate_check "$CMD"; then
            ...
            if command -v sentry_log >/dev/null 2>&1; then
                ...
                sentry_log "RATE_REPEAT" "$SENTRY_RATE_REASON" "claude-hook" "{\"cmd\":\"$safe_cmd\",\"cwd\":\"$CWD\"}" 2>/dev/null || true
            fi
        fi
    fi
    exit 0
